...
import { MouseTrailingEffect } from "./MouseTrailingEffect";

const SnakeGame = (): JSX.Element => {
  const emojiGoal = 5;
  const { width, height } = useWindowSize()
  const [displayConfetti, setDisplayConfetti] = useState(false)
  const [fireworksStatus, setFireworksStatus] = useState<'start' | 'pause' | 'stop'>('pause')
  const [possibleEmojis, setPossibleEmojis] = useState(
    emojis.slice(0, emojiGoal)
  );
  const [collectedEmojis, setCollectedEmojis] = useState(0); // TODO: use recoil to store the collected emojis
  ...
  return (
    <>
      {displayConfetti && (<Confetti style={{position: 'fixed'}} width={width} height={height} />)}
      <Fireworks status={fireworksStatus}/>
      <RandomEmoji onClick={spawnEmoji}>{emoji.char}</RandomEmoji>
      <MouseTrailingEffect string={tail} />
    </>
  );
};
